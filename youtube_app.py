...
def get_video_metadata(youtube_service, video_ids):
    ...
    for i in range(0, len(video_ids), 50):
        ...
        video_response = youtube_service.videos().list(
            id=','.join(chunk),
            # Add 'recordingDetails' to fetch location data
        ...
        for video in video_response.get('items', []):
            ...
            # Gracefully handle potentially missing location data
            recording_details = video.get('recordingDetails', {})
            location_info = recording_details.get('location', {})
            ...
            published_time = None
            if published_at:
                parts = published_at.split('T')
                published_date = parts[0]
                published_time = parts[1].replace('Z', '')

            video_id = video.get('id')
            details = {
                'Video ID': video_id,
                'Video URL': f"https://www.youtube.com/watch?v={video_id}" if video_id else None,
                'Title': snippet.get('title'),
                'Published Date': published_date,
                'Published Time (UTC)': published_time,
        ...
        try:
            ...
            if not video_ids_to_fetch:
                ...
                if not metadata:
                    ...
                    column_order = [
                        'Title', 'Channel Title', 'Published Date', 'Published Time (UTC)',
                        'View Count', 'Like Count', 'Duration', 'Video URL', 'Video ID',
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "Video URL": st.column_config.LinkColumn("Video Link", help="Click to open the video on YouTube", display_text="Open Video")
                        },
                        hide_index=True,
                    )
